...
const themes: Theme[] = [
    ...
    { name: "light-sky", isDark: false },
    { name: "bright-lights", isDark: true },
]
...
const ThemePallet: FC<ThemePalletProps> = ({ themeName, isCurrentTheme }) => (
    <button
        className={`pallet ${isCurrentTheme ? "pallet--selected" : ""}`}
        data-theme={themeName}
    >
        <h3 className='theme-pallet__name'>{themeName}</h3>
        ...
        <div className={`theme-pallet theme-${themeName}`}>
            ...
            <div
                className='theme-pallet__color'
                style={{ backgroundColor: "var(--brightRed)" }}
            />
        </div>
    </button>
)
...
const ThemesPage: FC = () => {
    ...
    const handleThemePalletClick = useCallback<
        React.MouseEventHandler<HTMLDivElement>
    >(({ target }) => {
        const candidate = target as HTMLElement
        const candidatePallet = candidate.closest(
            ".pallet"
        ) as null | HTMLElement

        if (candidatePallet) {
            const dataThemeName = candidatePallet.dataset.theme
            setTheme(dataThemeName)
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [])

    const mapThemes = useCallback(
        ({ name }: Theme) => {
            return (
                <ThemePallet
                    isCurrentTheme={name === currentTheme}
                    themeName={name}
                    key={name}
                />
            )
    ...
    return (
        <div className='themes-page' onClick={handleThemePalletClick}>
            <div className='themes-page__body'>
                <div className='themes-page__item themes-list'>
                    ...
                    <div className='themes-list__column'>
                        {themes.filter(({ isDark }) => !isDark).map(mapThemes)}
                    </div>
                </div>
                <div className='themes-page__item themes-list'>
                    <h3 className='themes-list__title'>
